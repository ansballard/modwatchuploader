...
class Application(Frame):
	...
	def getScriptVersion(self):
		...
		try:
			req = urllib2.Request("http://modwatchapi-ansballard.rhcloud.com/api/script/version")
	...
	def buildJSON(self):
		...
		fullParams = "{\"plugins\": "+pluginsToSend+",\"modlist\": "+modlisttxtToSend+",\"ini\": "+iniToSend+",\"prefsini\": "+prefsiniToSend+",\"game\":\"skyrim\", \"username\": \""+self.username+"\", \"password\": \""+self.password+"\"}"
		return fullParams

	def postLoadOrder(self, fullParams):
		try:
			#Fancy urllib2 things
			req = urllib2.Request("http://modwatchapi-ansballard.rhcloud.com/loadorder", fullParams, {'Content-Type':'application/json'})
			print 'Uploading to http://modwat.ch/u/' + self.username + '...'
			print fullParams
			#response = 200
			f = urllib2.urlopen(req)
			response = f.getcode()
	...
	def postNewPass(self):
		...
		req = urllib2.Request("http://www.modwat.ch/"+self.username+"/newpass", params, {'Content-Type':'application/json'})
